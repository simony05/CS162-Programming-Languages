...
module Infer = struct
  ...
  let rec abstract_eval (gamma : gamma) (e : expr) : ty =
    ...
    | ListMatch (e1, e2, binder) -> 
        ...
        let gamma1 = add gamma x x1 in
        let gamma2 = add gamma1 y (TList x1) in
        ...
        (
          ...
          | TVar x1, _ -> 
            if not (Vars.mem x1 (Utils.free_vars t2)) then 
              ...
              let o2 = [(x1, Utils.apply_soln o1 t2)] in
              o1 @ o2
            else ty_err ("error ")
          | _, TVar x2 ->
            if not (Vars.mem x2 (Utils.free_vars t1)) then 
              let o1 = unify (Utils.subst_cs x2 t1 cs') in
              let o2 = [(x2, Utils.apply_soln o1 t1)] in
              o1 @ o2
            else ty_err ("error ")
          | TFun(t11, t12), TFun(t21, t22) -> unify (cs' @ [(t11, t21); (t12, t22)])
          | TList t1, TList t2 -> unify (cs' @ [(t1, t2)])
          | TProd(t11, t12), TProd(t21, t22) -> unify (cs' @ [(t11, t21); (t12, t22)])
          | TSum(t11, t12), TSum(t21, t22) -> unify (cs' @ [(t11, t21); (t12, t22)])
          | _ -> ty_err ("fail ")
        )
end
...
let infer_with_gamma ~(gamma : gamma) (e : expr) : ty =
  ...
  Fmt.pr "Constraints: %a\n%!" Fmt.(vbox @@ list pp_cons) (Infer.curr_cons_list());
