...
let rec subst (x : string) (e : expr) (c : expr) : expr =
  ...
  | Fix binder -> let (y, exp) = binder in 
      if String.equal y x then Fix binder
    ...
    try
      ...
      *)
      ...
      | ListMatch (e1, e2, binder1) ->
            (match eval e1 with 
            | ListNil -> eval e2
            | ListCons(v1, v2)-> let (x, binder2) = binder1 in
                let (y, exp) =  binder2 in
                eval (subst x v1 (subst y v2 exp))
            (* substitute x::y with v1::v2 where v1 is head of e1 and v2 is remainder of e1 *)
            | _ -> im_stuck (Fmt.str "Unmatchable: %a" Pretty.expr e)
            )
      | Fix binder -> let (x, exp) = binder in let sub = subst x (Fix (x, exp)) exp in eval sub
